...
# Plot historical and forecasted magnitudes
...
# Add past earthquake locations with popups
def add_popup(lat, lon, magnitude):
    return folium.Popup(f"Latitude: {lat}<br>Longitude: {lon}<br>Magnitude: {magnitude}", parse_html=True)
...
for _, row in df.iterrows():
    folium.CircleMarker(
        ...
        fill_color='blue',
        fill_opacity=0.5,
        popup=add_popup(row['latitude'], row['longitude'], row['mag'])
    ).add_to(earthquake_map)

# Add predicted earthquake locations (random samples from past locations)
